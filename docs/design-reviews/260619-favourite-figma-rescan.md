...
# RE-GATE — mobile structural fixes (commit `4db98054`, PR #101)

**Date**: 2026-06-19 · **Author**: designer (re-gate of the FAIL above)
**Build**: branch `feat/favourite-figma-fidelity` · commit `4db98054`
  ("feat(favourite): message-hero card + drop Home caption row")
**Scope of THIS re-gate**: structural + token fixes that are determinable from
  the committed mobile code. Verification is **code-based** (cold-launch
  toolchain blocker + session-gate prevent a fresh populated live render;
  mcp-doctor exit 0 / sim healthy, but no warm logged-in Favourite session was
  up this pass). **OUT of scope / staged** (NOT failed here): the per-outfit
  MESSAGE TEXT and feeling CHIP only populate after the backend contract
  (`feat/favorites-title-mood`: GET/POST `title` + `mood_tags`) deploys AND
  users re-save — existing favourites stay null by CEO decision (degrade
  applies). I verify the client is correctly WIRED to display them when
  present; the "looks like Figma with real data" confirmation is a
  post-backend-deploy follow-up, not a blocker on the mobile structure.

## Verification of each prior finding

| Prior finding | Sev | Status | Evidence (code-based) |
|---|---|---|---|
| **MISS-1** — drop `OutfitCardCaption` from the favourite card | BLOCKER | ✅ RESOLVED | Import deleted (`FavouriteOutfitCard.tsx` import list); `caption`/`reasoning_human` local var removed; the `<OutfitCardCaption … />` render at the old `:155` is gone. `grep OutfitCardCaption src/screens/favourite/` → NONE. The component still exists for **Home** (correct — it was only removed from the favourite card, not globally). |
| **MISS-2** — no canned fallback; hero reads top-level `favourite.title`; clean degrade when null | BLOCKER | ✅ RESOLVED | Card reads `favourite.title?.trim()` (`:110`). Title block renders only `dateLabel \|\| title \|\| moodTagLabel` (`:128`); when `title` is empty the title `<Text>` **and both flanking `titleDivider`s** are omitted (`:135-149`) and **no caption is substituted** — the degrade branch is exactly as specced. `grep default_caption / 'Clean. Ready' src/` → only in the Home-scoped `OutfitCardCaption.tsx` + i18n strings, **never reachable from the favourite card** now. |
| **MISS-5** — title block reads as a centered column once the caption pill is gone | MAJOR | ✅ RESOLVED | `titleBlock { alignItems:'center', gap:xs, paddingVertical:s }` (`:229-233`); `date`, `title` both `textAlign:'center'`; dividers `alignSelf:'stretch'`. With the left-aligned caption pill removed, the centered date→divider→title→divider→chip column is the card hero. |
| **MISS-6** — divider `#f2f4f7` via new `figmaDividerSubtle` token | MINOR | ✅ RESOLVED | `titleDivider.backgroundColor = figmaDividerSubtle` (`:247`); token `figmaDividerSubtle: '#f2f4f7'` added (`theme.ts:48`). No bare `figmaDivider` (#D1D3D8) left in the card. |
| **MISS-7** — empty glyph `figmaTextDark` (#070707) | MINOR | ✅ RESOLVED | `EmptyState.tsx` glyph `color={theme.colors.figmaTextDark}` (was `uacTextBase`); `figmaTextDark: '#070707'` (`theme.ts:69`). |
| Tile radius → `figmaTile` (12) | MINOR (B2) | ✅ RESOLVED | `tile.borderRadius = borderRadius.figmaTile` (`:283`); `figmaTile: 12` (`theme.ts:348`). Home/Favourite tile parity per CEO 2026-06-19. |
| Header hamburger→drawer | (was ESCALATE F1) | ✅ CEO-CONFIRMED — NOT a finding | Per re-gate dispatch: CEO confirmed the hamburger→drawer header stays. Closed; no longer open. |

## Wiring check (display-when-present, the staged half)

- Save model `SaveFavouritePayload.title?: string` (`favouriteService.ts:30`) +
  read model `title?: string \| null` (`:83`) and `mood_tags` are on the model —
  the card reads `favourite.title` / `favourite.mood_tags[0]` already.
- Save sites thread the V05 message: `HomeScreen` direct-save
  (`...(outfit.caption ? { title } : {})`) + mood-sheet save (same) + the mood
  hook ref `MoodFeedbackOutfitRef.title` → `useMoodFeedback` POST. Backward-safe:
  `title` omitted (not `""`) when absent; backend ignores until the column ships.
- **Conclusion**: the client renders the hero message + feeling chip the moment
  the backend returns them. No further mobile change needed for visual parity.

## Token / drift backstop

`auxi-lint-tokens.sh`: 32 pre-existing repo violations, **zero in any
commit-touched file** (`FavouriteOutfitCard.tsx`, `favourite/EmptyState.tsx`,
`theme.ts`, `favouriteService.ts`, `use-mood-feedback.ts`, and the HomeScreen
save-path lines). This commit adds no hex/font drift; the new tokens are real
`theme.ts` entries.

## RE-GATE VERDICT

**PASS (mobile structural fixes)** — all prior BLOCKERs (MISS-1, MISS-2) and all
prior MINORs (MISS-6, MISS-7, tile radius) are **resolved in code**; MISS-5
centering is correct once the caption row is gone; the header ESCALATE is
CEO-closed. No new token drift. The PR #101 mobile structure is on-system.

**Pending, NOT a blocker on the mobile structure (follow-up, post-backend-deploy):**
final visual fidelity ("looks like Figma with real data" — distinct per-outfit
messages + per-card feeling chips) requires the backend `feat/favorites-title-mood`
contract to deploy AND users to re-save. Existing favourites stay null by CEO
decision and use the clean degrade. Re-confirm a populated live render
(lenses 1/3/8) after backend deploy + a re-save.
