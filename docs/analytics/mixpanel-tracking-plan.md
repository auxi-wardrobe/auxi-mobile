...
| `body_shape_generation_failed` (AU-358 3-shape) | The 3-shape generation job failed or timed out (poll) | `SeeThisOnMeScreen.tsx` (resolution effect) | `outfit_hash` |
...
| `notifications_schedule_changed` | Repeat Schedule picker save — Weekdays/Everydays (frequency) cadence saved (AU-316; previously also carried `period` for the AM/PM editor, which was removed in the Settings IA redesign). Fires on persist success | `SettingsScreen.tsx:541` | `frequency` (`weekdays`/`everydays`) |
| `notifications_reset` | "Reset to default setting" link applied — daily-notification block restored to first-run defaults (AU-316 RST-1). Fires on persist success. **Note:** previously carried `period` (`AM`) + `frequency` (`weekdays`); `period` dropped when the AM/PM editor was removed | `SettingsScreen.tsx:617` | `frequency` (`weekdays`) — the default applied |
| `notifications_reset_undone` | Undo tapped on the post-reset snackbar — prior notification values restored (AU-316 RST-1). **Note:** previously carried `period` + `frequency`; `period` dropped when the AM/PM editor was removed | `SettingsScreen.tsx:565` | `frequency` — the restored prior value |
...
| `settings_section_opened` | Settings root group-row tapped → navigates to a sub-screen (Personalization / Privacy / About). Fired from `SettingsScreen.tsx` `openSection` handler on each group row's `onPress` | `SettingsScreen.tsx` (`openSection`) | `section` (`personalization` / `privacy` / `about`) |

> PII: all settings events carry bounded enums only (`frequency`, `direction`, `locale`, `granted`, `enabled`, `section`) — no raw text, no identifiers. The read-only hour value (`'06:15'`) is NOT tracked. `notifications_reset` props echo the constant defaults (so the dashboard can confirm what "default" was at fire time); `notifications_reset_undone` echoes the restored prior value for symmetry.
>
> **Breaking change (Settings IA redesign):** `notifications_schedule_changed`, `notifications_reset`, and `notifications_reset_undone` previously carried a `period` property (`AM`/`PM`) from the AM/PM time-of-day editor. That editor was removed; all three events now carry `{ frequency }` only. Mixpanel dashboards or funnels that filter or break down on `period` must be updated — historical events with `period` remain in the event stream but new events will never carry it.
