...
| Event | Trigger | Location | Properties |
|---|---|---|---|
| `discovery_feed_viewed` | `DiscoveryScreen` gains focus — once per focus (mount + every re-focus, not on every in-session filter tweak) | `src/hooks/useDiscoveryFeed.ts` (`useFocusEffect`) | `filter_season?`, `filter_trend_tag?` (both omitted when no filter active), `wardrobe_gender?` (`M`/`W`/`U` — the gender the BACKEND applied, read from the feed response's `applied_gender`; **omitted on a cold first focus**, because the focus event fires before the first query resolves, and omitted for a user with no onboarding direction on file) |
| `discovery_filter_applied` | A season or trend-tag chip is tapped (including clearing back to "All") | `src/hooks/useDiscoveryFeed.ts` `onSeasonChange` / `onTrendTagChange` | `filter_type` (`season` \| `trend`), `filter_value` (season enum / tag, or `all`) |
