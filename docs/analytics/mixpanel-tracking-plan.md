...
| `wardrobe_direction_selected` | Direction pick | `OnboardingWardrobeScreen.tsx:56` | `direction` (`Menswear` / `Womenswear` — the `Mixed` tile was retired 2026-09-23, plan 260923; historical `Mixed` values stop after that date) |
...
| `recommendation_exhausted` | Plan 260923 — the server reports every distinct outfit (primary = top+bottom / one-piece) the user's wardrobe makes for this weather + occasion has been shown (`exhausted=true` on build or try_another). Fires once per exhaustion episode (ref-guarded; re-armed when new outfits land or on start-over). The server no longer re-serves seen outfits or pads with catalog items, so this is the honest "no more options" moment. | `HomeScreen/index.tsx` (`startMutation.onSuccess`) | `occasion` (active mode: `safe` / `power` / `creative`); `outfits_seen` (number — distinct outfits shown in this context, server count; omitted when unknown) |
| `recommendation_seen_reset` | User taps "Start over" — on the exhausted empty state (`HomeExhaustedState`) or the exhausted snackbar action — to forget what was shown for this weather + occasion and rebuild with `reset_seen=true`. | `HomeScreen/index.tsx` (`handleStartOver`) | `occasion` |
...
| | | | **Also:** two beautify-status branches carry `is_common` plus an additional `beautify_status` (`'pending'` \| `'ready'`) dimension — `is_common` is present on all three branches (default + both beautify), never omitted |
...
| `body_photo_reuse_confirmed` (AU-354 pt.3; `auto` added when the confirm step was removed for shape-carrying profiles) | On re-entry with a saved reusable profile, the persisted body is reused → proceeds to render (no re-capture). `auto:true` = the profile already carried a `body_shape`, so the sheet was SKIPPED entirely and the flow went straight to the render loading screen (picking the shape was the confirmation); `auto:false` = the user tapped Confirm on the reuse-confirm sheet, which now only appears for a profile with no saved `body_shape` | `SeeThisOnMeConfirmScreen.tsx` (`handOffRender`) | `outfit_hash`, `auto` |
...
| `body_shape_generation_quit_prompt_shown` (AU-358 quit sheet) | Back (header chevron / Android hardware back) pressed while a body-shape/render generation is live → the discard-vs-notify confirm sheet (`DiscardGenerationDialog`) is shown instead of silently backgrounding. Funnel head for the quit-during-generation branch | `SeeThisOnMeScreen.tsx` (`handleBackDuringGeneration`) | `outfit_hash` |
| `body_shape_generation_discarded` (AU-358 quit sheet) | User chose **Discard** on the quit confirm sheet → the in-flight job is cancelled (`tryOnGenerationStore.reset()` orphans the poll), no completion toast, user returns home | `SeeThisOnMeScreen.tsx` (`handleDiscardGeneration`) | `outfit_hash` |
| `body_shape_generation_quit_cancelled` (AU-358 quit sheet) | User chose **Keep generating** / dismissed the quit confirm sheet → stays in the flow, job untouched. Distinguishes "changed my mind, keep waiting" from the discard / notify exits | `SeeThisOnMeScreen.tsx` (`handleQuitCancel`) | `outfit_hash` |
...
| `ai_limit_gate_shown` (AI daily-limit gate) | The daily AI-budget limit sheet appeared on a `429 ai_daily_limit_reached` — replaces the generic "Try again" error view (kills the retry-storm). The `render`/`shapes` phases fire once per limit-resolution (deduped by the same `resolvedHashRef` key that guards `try_on_failed` / `body_shape_generation_failed`), so they do NOT double-count on re-render. The `entry` phase fires when See-on-me opens already knowing the limit is reached (a remembered prior 429, via `aiLimitStore`) and shows the sheet up front — no capture flow, no AI job started. Terminal "hit daily limit" state — there is no retry from the sheet | `SeeThisOnMeScreen.tsx` render + shapes resolution effects and the mount entry-gate, via `useAiLimitGate.ts` + `aiLimitStore.ts` | `feature` (`try_on`), `phase` (`render` / `shapes` / `entry`) — bounded enums, no PII |
...
| `wardrobe_direction_changed` | Plan 260923 — Settings › Personalization › Wardrobe: Update saved a NEW direction (`PUT /me/wardrobe-direction` succeeded; picking the current value is a no-op and does not fire). The server re-seeds the starter items and Discovery re-filters. | `settings/SettingsPersonalizationScreen.tsx` (`applyWardrobe`) | `from` (`menswear` / `womenswear` — omitted when the user had no valid direction, e.g. a legacy `Mixed` or unset); `to` (`menswear` / `womenswear`) |
...
- **Wardrobe exhaustion (plan 260923):** `recommendation_exhausted` → `recommendation_seen_reset` (start over) vs `add_item_opened` / `wardrobe_item_added` (grows the wardrobe) vs silent exit. `recommendation_exhausted` ÷ users reaching Home is how often wardrobes run dry for a weather + occasion — break down by `occasion` and bucket `outfits_seen` to see how many distinct looks a wardrobe offers before running out (small numbers = a wardrobe-growth opportunity, not an engine bug). `recommendation_seen_reset` ÷ `recommendation_exhausted` is the replay rate.
- **Wardrobe direction switches (plan 260923):** `wardrobe_direction_changed` broken down by `from` → `to`, as a share of Settings `screen_viewed`. After the Mixed retirement, a burst of switches from former-Mixed users (`from` omitted) is expected; sustained switching afterwards signals the onboarding pick is misunderstood.
...
- **Try-on funnel:** `try_on_started` → `try_on_step_completed` ×N → `try_on_completed` → `try_on_outcome_retaken` *(extend to `_saved`/`_shared` when UI ships)*. **AU-358 note:** the render is now async (submit → poll) and so is the body-shape step — both run in the out-of-React generation store, so `try_on_started`/`try_on_completed` now bracket a worker poll, not a synchronous call. **Daily-limit terminal state:** `ai_limit_gate_shown` (`feature = try_on`, `phase = render`/`shapes`/`entry`) is the "user hit today's AI budget" exit — it replaces `try_on_failed` / `body_shape_generation_failed` on a `429 ai_daily_limit_reached` and offers no retry. `ai_limit_gate_shown` ÷ `try_on_started` is the daily-cap block rate; segment by `phase` to see whether users are capped at the body-shape gen (`shapes`) or the final render (`render`), vs. gated **at entry** (`entry`) — the last is the proactive path where a remembered prior 429 shows the sheet the instant See-on-me opens, sparing the user the capture + selection walk. **See-on-me redesign (stepped shell, B1–B3):** `try_on_step_viewed` (`step`: `selfie`/`full_body`/`body_fit`) now brackets each stepped capture screen becoming active — drop-off between consecutive `try_on_step_viewed` values is the per-step abandon rate. The funnel gains a **result-satisfaction step** at the tail: `try_on_completed` → `try_on_result_liked` **or** `try_on_result_disliked` (mutually exclusive per result, vote changeable). `(liked + disliked) ÷ try_on_completed` is the feedback-response rate; `liked ÷ (liked + disliked)` is result satisfaction. Server-side persistence of the vote (`POST /api/tryon/feedback`) is a backend follow-up (§6.4) — the client-side event fires regardless.
- **3-shape generation sub-funnel (AU-358):** `body_shape_generation_started` → `body_shape_generation_completed` (`partial` branch) → `body_shape_selected` → `try_on_started`. Drop-off at `_completed` measures AI body-shape gen reliability; `body_shape_generation_failed` ÷ `_started` is the failure rate. **Quit-during-generation sub-branch (AU-358 quit sheet):** pressing back on a live loading screen fires `body_shape_generation_quit_prompt_shown` (the discard-vs-notify sheet head), which splits three ways — `body_shape_generation_discarded` (cancel the job, no toast), `body_shape_generation_backgrounded` (keep it running + notify, recovered via `body_shape_generation_completed_notified`), or `body_shape_generation_quit_cancelled` (dismiss the sheet, keep waiting in-flow). `body_shape_generation_discarded` ÷ `body_shape_generation_quit_prompt_shown` is the abandon rate once the quit prompt appears.
- **Reuse-on-return funnel (AU-354 pt.3):** on re-entry with a saved reusable body profile, `body_photo_reuse_confirmed` → `try_on_started` → `try_on_completed` measures returning-user conversion; `body_photo_retake_selected` is the drop-to-recapture branch (denominator: arrivals on the reuse-confirm screen). The reuse-confirm step is now a `ContextualBottomSheet`, so it has THREE mutually-exclusive exits off that denominator: `body_photo_reuse_confirmed` (proceed), `body_photo_retake_selected` (recapture), and `body_photo_reuse_dismissed` (backdrop-tap / swipe-down — leaves the flow). `body_photo_reuse_dismissed` ÷ arrivals is the silent-abandon rate that was invisible before the sheet dismiss was tracked.
  **Segment this funnel on `auto` from here on.** The confirm sheet was removed for profiles that already carry a `body_shape` (the shape pick is the confirmation), so those entries now emit `body_photo_reuse_confirmed` with `auto:true` and go straight to the render — they never arrive on the sheet and can never emit `retake_selected` / `reuse_dismissed`. Since `select` always sets `body_shape`, expect `auto:true` to be nearly all of the volume: the three-way exit split above is only meaningful over `auto:false` (legacy no-shape profiles), and the drop in sheet arrivals is the intended change, not a regression.
...
- **Beautify funnel:** `beautify_started` → `beautify_ready` → `beautify_review_opened` → `beautify_accepted`. Drop-off between `beautify_started` and `beautify_ready` = job failure / timeout rate (see §6.7 gap — `beautify_failed` not yet wired so failures are only visible as missing continuations). `beautify_wait_continued_browsing` between `started` and `ready` is the leave-during-wait branch — segment `beautify_review_opened` by `from` (`loader` / `snackbar` / `push` / `tile`) to compare users who watched the full loader, returned via the Wardrobe snackbar, tapped a push notification on job completion, or tapped the wardrobe tile directly once already `ready`. `beautify_kept_original` and `beautify_regenerated` are exits or re-entry loops from the review step; `beautify_regenerated` broken down by `source` (`review` vs `retry_pending`) distinguishes deliberate re-rolls from failure-recovery retries. `add_item_mode_selected { mode: 'beautify' }` is historical only because the upload-time selector was removed.
...
- **Discovery browse-to-conversion funnel (AU-457, §5.27):** `discovery_feed_viewed` → `discovery_outfit_opened` → `discovery_see_on_me_tapped` **and/or** `discovery_item_saved` (the two conversions are independent, not mutually exclusive — a user can save an item AND try the outfit on). `discovery_outfit_opened ÷ discovery_feed_viewed` is feed engagement; `(discovery_see_on_me_tapped + discovery_item_saved) ÷ discovery_outfit_opened` is the outfit-detail conversion rate. Break down `discovery_filter_applied` by `filter_type`/`filter_value` to see which season/tag combinations drive engagement (both axes are multi-select, so `filter_value` is the comma-joined selection — a combination arrives as ONE value, e.g. `summer,winter`, rather than as two events). `discovery_deep_link_opened` (`resolved: true` vs `false`) measures social-post link health independent of the on-app browse path. **`discovery_feed_empty` is a coverage ALARM, not a funnel step** — segment it by `wardrobe_gender` to find a blacked-out cohort. Strict gender targeting means a gender with no published outfits gets a silently empty feed, so a spike on one `wardrobe_gender` value means the Discovery board needs outfits tagged for that wardrobe (the admin SPA's `/gender-coverage` banner shows the same thing from the curation side). Also worth cutting `discovery_feed_viewed` by `wardrobe_gender` to confirm each cohort's engagement separately — a healthy overall funnel can hide one starved cohort.
...
| `beautify_review_opened` | BeautifyReview screen mounts — fired via `useFocusEffect`-equivalent `useEffect` on mount. `from` distinguishes the four entry points: the loader auto-advance, the Wardrobe snackbar tap (user left the loader and returned via the "Studio shot ready — Review" snackbar), a push-notification tap (`push` — opened via push-notification tap on job completion), or a direct wardrobe-grid tile tap (`tile` — opened via direct wardrobe-tile tap while status was already `ready`) | `BeautifyReviewScreen.tsx:35` | `from` (`loader` / `snackbar` / `push` / `tile`) |
...
| `capsule_build_method_selected` | A build path chosen on the method step — on the row tap for `ai` (Coming soon, nothing is created), on Next for the path used | `CapsuleMethodScreen.tsx` | `method` (`manual` / `ai`) |
| `capsule_created` | "Build it myself" → empty capsule POSTed successfully | `CapsuleMethodScreen.tsx` | `method`, `status` |
| `capsule_configured` | Create tapped on the requirements step — **parked with the AI path, not currently reachable** | `CapsuleInfoScreen.tsx` | `has_temp_range`, `formalness_level?`, `outfit_target?`, `shoe_limit?` |
| `capsule_generation_started` | Create mutation fires on the generating screen — **parked with the AI path** | `CapsuleGeneratingScreen.tsx` | `outfit_target?` |
...
| `capsule_settings_edited` | Capsule name saved via the edit screen | `CapsuleEditScreen.tsx` | `changed_constraints` (bool — **always false today**: the edit screen is name-only while AI generation is Coming soon) |

> Design revision 260729 (build-method step): capsule creation is now name → **build method** → empty capsule. Only "Build it myself" builds anything (`capsule_created`); "Let AI build it" is Coming soon and its tap only emits `capsule_build_method_selected { method: 'ai' }` as a demand signal. The requirements + generating screens (and their `capsule_configured` / `capsule_generation_*` events) are parked until the AI path ships — they stay registered so re-enabling is a routing change, not a rewrite.
>
...
### 5.27 Discovery (AU-457)

Admin-curated multi-item outfit browsing (`GET /api/discovery/outfits`, `GET /api/discovery/outfits/{id}`, `GET /api/discovery/trend-tags`), reached from a `SidebarMenu` row (D1 — drawer row + stack route, no bottom tab-bar this round). From an outfit, users "See on me" (real AI try-on via the existing `SeeThisOnMeConfirm` reuse-confirm gate — the try-on hash is namespaced `discovery_{outfit_id}` so it never collides with a favourite/creation hash), tap an item for detail, and save an item to their wardrobe via the existing `POST /wardrobe/common-items/{item_id}/clone` (`useSaveCommonItemToWardrobe.ts`). A social post can deep-link straight into one outfit (`deepLinkHandler.ts`, kind `discovery-outfit`). Direct `track()` calls (no dedicated wrapper functions — none of these need cross-file reuse or session dedup beyond what's noted below). **PII: none — `outfit_id` / `item_id` are internal DB ids; `filter_value` is a closed season enum or an admin-authored trend tag, never free text.**

**Gender targeting (2026-09-12).** The feed and the trend-tag chip row are filtered SERVER-side to the user's wardrobe gender, derived from their persisted onboarding direction (`user_metadata.wardrobe_direction`) via the AU-305 rule: `Menswear` sees only `M` outfits, `Womenswear` only `W`, `Mixed` all three, and a user with no direction on file sees everything unfiltered. The client sends nothing for this and cannot override it — `wardrobe_gender` on these events is the value the backend *reported back* as `applied_gender`, so segmenting by it segments by what users actually saw, not by what the app guessed. `wardrobe_gender` is a single-character bucket, never PII.

| Event | Trigger | Location | Properties |
|---|---|---|---|
| `discovery_feed_viewed` | `DiscoveryScreen` gains focus — once per focus (mount + every re-focus, not on every in-session filter tweak) | `src/hooks/useDiscoveryFeed.ts` (`useFocusEffect`) | `filter_season?`, `filter_trend_tag?` (both axes are multi-select, so each is a comma-joined list — `"summer,winter"` — and both are omitted when no filter is active), `wardrobe_gender?` (`M`/`W`/`U` — the gender the BACKEND applied, read from the feed response's `applied_gender`; **omitted on a cold first focus**, because the focus event fires before the first query resolves, and omitted for a user with no onboarding direction on file) |
| `discovery_filter_applied` | A season or trend-tag selection is committed with "Show" in its filter sheet (including clearing back to "All") — once per apply, not once per chip tap | `src/hooks/useDiscoveryFeed.ts` `onSeasonsChange` / `onTrendTagsChange` | `filter_type` (`season` \| `trend`), `filter_value` (the whole selection for that axis, comma-joined in canonical order — `"spring,fall"` — or `all` when empty) |
| `discovery_outfit_opened` | A feed card is tapped | `src/screens/discovery/DiscoveryScreen.tsx` `handleOutfitPress` | `outfit_id`, `position` (0-based grid index), `source: 'feed'` |
| `discovery_see_on_me_tapped` | "See on me" sticky CTA tapped on the outfit detail screen (only when 1–4 live items — see D2) | `src/screens/discovery/DiscoveryOutfitDetailScreen.tsx` `handleSeeOnMe` | `outfit_id`, `item_count` |
| `discovery_item_saved` | `POST /wardrobe/common-items/{item_id}/clone` succeeds from the outfit detail's item strip → `ItemDetail` | `src/hooks/useSaveCommonItemToWardrobe.ts` | `outfit_id`, `item_id` |
| `discovery_feed_empty` | The feed settles with `total === 0` AND no filter is active — i.e. this user's whole Discovery feed is empty. Fires **once** per empty result set (ref-guarded so a re-render or background refetch can't inflate the count); resets when the feed becomes non-empty. Deliberately does NOT fire when a season/tag filter returns nothing — that's a normal user action, not a coverage failure | `src/hooks/useDiscoveryFeed.ts` (settle effect) | `wardrobe_gender?` (omitted when the feed was unfiltered) |
| `discovery_deep_link_opened` | A `discovery-outfit` deep link lands on `DiscoveryOutfitDetailScreen` (`source: 'deep_link'` route param, set only by `deepLinkHandler.ts`) and the outfit fetch settles — fires once per outfit id, never for a plain feed-tap open (that's `discovery_outfit_opened` above) | `src/screens/discovery/DiscoveryOutfitDetailScreen.tsx` (settle effect) | `outfit_id`, `resolved` (`true` on a live outfit, `false` on the identical-envelope 404 or a transport error) |
