...
`wardrobe_item_deleted` (§5.4) does not distinguish removing one of Macgie's seeded default items from removing an item the user added themselves. The screen knows (`isDefaultItem`, `ItemDetailScreen.tsx`), so the property is a one-line add — deferred because the backend's `is_default_item` column segments it server-side today. Note the `USR_` hrid prefix is **not** a substitute: Database picks share it with seeded defaults.
...
- **Wardrobe-ownership milestone (§5.27):** `default_items_unlock_celebrated` → `default_items_unlock_dismissed`. **Not a step funnel off `add_item_upload_succeeded`** — the milestone fires on wardrobe growth from any route (upload OR Database pick), so an upload-anchored funnel would under-count every user who crossed the line by picking. Use the celebrated event as its own cohort marker instead. This is the "user's wardrobe is now genuinely theirs" moment — the count of `default_items_unlock_celebrated` over a cohort is how many users reach a self-sufficient wardrobe, and time-to-event from `sign_up_completed` is how long it takes them. Because it fires once per user ever, it doubles as an activation-depth marker: segment retention and `outfit_favorited` rates by whether a user has celebrated to test the hypothesis that a real (non-default) wardrobe drives the value moment. Break down by `own_item_count` to see how many users overshoot the threshold before the sheet catches them. `default_items_unlock_dismissed` ÷ `default_items_unlock_celebrated` should be ≈1 — a persistent gap means users are backgrounding the app on the milestone sheet.
...
Macgie seeds every new wardrobe with default (starter catalog) items so the suggestion engine has signal on day one. They are immutable until the user owns enough items of their **own** — the backend owns the threshold (currently 12) and reports it via `GET /api/wardrobe/default-items/removal-status`. The moment a user crosses it we congratulate them **once** and tell them the defaults can go.

"Own" spans every route by which a user acquires an item: uploading a photo (camera / gallery / web import) **and** picking a catalog item themselves from the Database screen or a trending drop. Only the onboarding-seeded defaults are excluded.

Guard logic lives entirely in `src/services/defaultItemsMilestone.ts`'s `maybeCelebrateDefaultItemsUnlocked` — same shape as `usageLimit.ts`: fire-and-forget, never blocking the success UI, failing open silently on any network/storage error (no event fires). Once-per-user is persisted in AsyncStorage (`default_items_unlock_celebrated_{userId}`), not session memory, because the wardrobe can dip back under the threshold on a delete and cross it again.

**Trigger point is the wardrobe list, not an upload handler.** `useDefaultItemsUnlockedCelebration` (WardrobeScreen) re-checks whenever the wardrobe's size grows. That is deliberate: the 12th item can arrive from four routes, and the Database-pick route never runs through `useAddWardrobeItem` — it navigates back to Wardrobe from its own screen. Hanging the check off `add_item_upload_succeeded` would silently miss it. Consequence for analysis: `default_items_unlock_celebrated` does NOT reliably follow `add_item_upload_succeeded` in the same session step — do not build a funnel that assumes it does (see §10).

**PII: none — only integer counts.** `own_item_count` / `threshold` are wardrobe sizes, not identifiers.

| Event | Trigger | Location | Properties |
|---|---|---|---|
| `default_items_unlock_celebrated` | `GET /wardrobe/default-items/removal-status` resolves `unlocked: true` for a user who hasn't been congratulated yet — fires once per user, ever | `src/services/defaultItemsMilestone.ts` `maybeCelebrateDefaultItemsUnlocked`; called from `useDefaultItemsUnlockedCelebration.ts` (WardrobeScreen, on wardrobe-size growth) | `own_item_count` (int), `threshold` (int) |
| `default_items_unlock_dismissed` | `DefaultItemsUnlockedSheet` dismissed ("Got it" or backdrop) | `useDefaultItemsUnlockedCelebration.ts` (`sheetProps.onDismiss`) | — |
...
>
> The removal gate itself (the ItemDetail Trash button on a default item) is **not** separately tracked — a successful removal already fires `wardrobe_item_deleted` (§5.4). Isolating default-item removals server-side needs the `is_default_item` column: the `USR_` hrid prefix does NOT work, because a catalog item the user picked themselves carries the same prefix. See §6.10.
