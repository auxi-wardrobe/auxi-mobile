...
- **Cross-subdomain cookie works.** `pages.dev` is on the Public Suffix List, so `auxi-web-review.pages.dev` is the registrable domain. A cookie with `Domain=auxi-web-review.pages.dev` is sent to **all** `*.auxi-web-review.pages.dev` subdomains. (`Domain=pages.dev` would be blocked — we never use that.)
...
- **Isolation:** ephemeral mode + browser storage partitioning keep impersonation from polluting the designer cookie or bleeding between different user-detail iframes.
