...
const selectStyle: React.CSSProperties = {
  padding: '8px 10px', borderRadius: 8, border: 'none', fontSize: 14,
...
export const DeviceFrame: React.FC = () => {
  ...
  const controls = (
    <div data-testid="review-controls" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <select data-testid="screen-select" value={screenKey} onChange={onScreen} style={selectStyle}>
      ...
      <button data-testid="copy-link" onClick={onCopy}
        style={{ ...btnStyle, background: copied ? '#34c759' : '#fff', color: copied ? '#fff' : '#000' }}>
        {copied ? 'Copied!' : 'Copy link'}
      </button>
      {!isMobile && (
        <select data-testid="device-select" value={idx} onChange={onDevice} style={selectStyle}>
          {DEVICES.map((dev, i) => (
  ...
  const toggle = (
    <button data-testid="controls-toggle" onClick={() => setOpen((o) => !o)}
      style={{ ...btnStyle, position: 'fixed', top: 'calc(10px + env(safe-area-inset-top))', right: 10, zIndex: 20,
        background: open ? '#fff' : 'rgba(28,28,30,0.92)', color: open ? '#000' : '#fff',
  ...
  const panel = open ? (
    <div style={{ position: 'fixed', top: 'calc(56px + env(safe-area-inset-top))', right: 10, zIndex: 20,
      background: '#1c1c1e', padding: 12, borderRadius: 12, boxShadow: '0 8px 40px rgba(0,0,0,0.5)', maxWidth: '82vw' }}>
      {controls}
    </div>
  ) : null;
  ...
  if (isMobile) {
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#000', overflow: 'hidden' }}>
        <iframe key={screenKey} title="auxi web review" src={iframeSrc()}
          style={{ width: '100vw', height: '100dvh', border: 'none', display: 'block', background: '#fff' }} />
        {toggle}
        {panel}
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#1c1c1e', display: 'flex', alignItems: 'center',
      justifyContent: 'center', padding: 16, boxSizing: 'border-box', overflow: 'auto' }}>
      {toggle}
      {panel}
      <iframe key={`${idx}-${screenKey}-${bleed}`} title="auxi web review" src={iframeSrc()} style={{
