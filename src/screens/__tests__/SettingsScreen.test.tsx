...
describe('applyChangeTime (reminder time / period)', () => {
  ...
  it('failure: dialog stays open, committed value unchanged, error toast, no unhandled rejection', async () => {
    ...
    expect(isDialogOpen(root, 'settings-time-update')).toBe(true);
    // committed display value unchanged — row value still shows default "06:15 AM"
    const value = oneByTestID(root, 'settings-time-row').findAll(
      n =>
        typeof n.type === 'string' &&
        typeof n.children?.[0] === 'string' &&
        n.children[0].includes('AM'),
    );
...
describe('applyChangeSchedule (repeat schedule / frequency)', () => {
  it('success: only frequency persisted, dialog closes', async () => {
    const updateCurrentUser = jest.fn().mockResolvedValue(
      makeUser({
        daily_notification: {
          enabled: true,
...
describe('handleResetPreferences', () => {
  it('is_first_login=false → modal closes', async () => {
